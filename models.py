...
class User (db.Model):
    ...
    def __repr__ (self):
        return '<User fullname: {} {}, email: {}>'.format(self.first_name, self.last_name, self.email)
...
class Task(db.Model):
    ...
    isComleted = db.Column(db.Boolean, default =False)

    user_id = db.Column (db.Integer, ForeignKey('user.user_id'))
    user= db.relationship ('User', back_populates='tasks')

    priority_id = db.Column(db.Integer, ForeignKey('priority.priority_id', name='fk_priority'))
